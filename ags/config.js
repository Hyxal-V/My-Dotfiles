...
var cpu = Variable("", {
    poll: [2000, `/home/<usrname>/.config/ags/checkCPU.sh | sed 's/\\//g'`],
})
...
const Player = () => Widget.Box({
    ...
    children:[
      Widget.Button({
          ...
          onClicked:()=>{
            MediaPlayerVisibility.value = !MediaPlayerVisibility.value
   ...
   self.label = player.getPlayer()==null?"No Media Detected":(`${player.getPlayer().track_title.slice(0, 30) + (player.getPlayer().track_title.length > 30 ? '...' : '')}`);
    ...
    hpack:"center",
        ...
        children:[
            Widget.Label({
                ...
                class_name:"ClockLarge",
                label:time24.bind()
          }),
      Widget.Label({
        justification: 'center',
        class_name:"DateLarge",
        label:date.bind(),
        hpack:'start',
...
App.config({
    style: "/home/<usrname>/.config/ags/style.css",
    windows: [
        bar,
        Time
